...
def ensure_predicted_high_column(engine, table_name):
    metadata = MetaData()
    try:
    ...
    if 'predicted_high' not in table.c:
        # Correctly wrap the ALTER TABLE statement in text() for execution
        alter_stmt = text(f"ALTER TABLE {table_name} ADD COLUMN predicted_high FLOAT;")
        with engine.begin() as conn:  # Use a transaction for DDL statements
            conn.execute(alter_stmt)
        logger.info(f"Column 'predicted_high' added to {table_name}.")
        return True
    return False
...
def save_predictions_to_db(predictions, crypto_symbol, timestamps):
    ...
    # Ensure the predicted_high column exists
    if not ensure_predicted_high_column(engine, table_name):
        return
...
def save_metadata(best_model, X, y, crypto_symbol, chosen_objective):
    ...
    metadata_path = os.path.join(output_dir, f"{crypto_symbol}10mi_{chosen_objective}_high_metadata.json")
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=4, cls=NumpyEncoder)
    logger.info(f"Metadata saved for {crypto_symbol}.")
    model_filename = os.path.join(models_dir, f"{crypto_symbol}10mi_{chosen_objective}_high_model.joblib")
...
def preprocess_data(df, crypto_symbol):
    # Separate the timestamp and 'high' columns
...
def process_crypto(crypto_symbol, chosen_objective):
    model_filename = os.path.join(models_dir, f"{crypto_symbol}10mi_{chosen_objective}_high_model.joblib")
    study_name = "10min_" + chosen_objective + "_high_" + crypto_symbol
    ...
    X = df.drop(['timestamp', 'high', 'future_high'], axis=1)
    y = df['future_high']
