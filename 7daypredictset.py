...
high_dir = './high'
low_dir = './low'
predictions_dir = './predictions_set'
...
def predict_and_save(crypto_symbols, model_types, days=7):
    for symbol in crypto_symbols:
        ...
        for model_type in model_types:
            model_set = 7  # You can change this as needed
            csv_file_path = os.path.join(predictions_dir, f"{symbol}_5min_{model_type}_{model_set}_predictions.csv")
            ...
            # Save each set of models in their own files
            models = load_models_for_type(model_type)
            if not models or symbol not in models:
                logger.error(f"No models found for {model_type} for {symbol}")
                continue
            model_output_dir = close_dir if model_type == 'close' else (high_dir if model_type == 'high' else low_dir)
            model_output_dir = os.path.join(model_output_dir, symbol)
            os.makedirs(model_output_dir, exist_ok=True)
            for model_info in models[symbol]:
                _, model, model_file_name = model_info
                for i in range(8):  # Save 8 files per model type
                    model_output_path = os.path.join(model_output_dir, f"{model_file_name.split('.')[0]}_{i}.joblib")
                    joblib.dump(model, model_output_path)
                    logger.info(f"Saved model {model_file_name} for {symbol} in {model_output_path}")
