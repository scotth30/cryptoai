...
def save_metadata(best_model, X, y, crypto_symbol, chosen_objective):
    ...
    metadata_path = os.path.join(output_dir, f"{crypto_symbol}10mi_{chosen_objective}_low_metadata.json")
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=4, cls=NumpyEncoder)
    logger.info(f"Metadata saved for {crypto_symbol}.")
    model_filename = os.path.join(models_dir, f"{crypto_symbol}10mi_{chosen_objective}_low_model.joblib")
...
def preprocess_data(df, crypto_symbol):
    # Separate the timestamp and 'low' columns
...
def process_crypto(crypto_symbol, chosen_objective):
    model_filename = os.path.join(models_dir, f"{crypto_symbol}10mi_{chosen_objective}_low_model.joblib")
    study_name = "10min_" + chosen_objective + "_low_" + crypto_symbol
    ...
    X = df.drop(['timestamp', 'low', 'future_low'], axis=1)
    y = df['future_low']
