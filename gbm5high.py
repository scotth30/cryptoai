...
def save_metadata(best_model, X, y, crypto_symbol):
    ...
    metadata_path = os.path.join(output_dir, f"{crypto_symbol}5mi_rmse3_high_metadata.json")
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=4, cls=NumpyEncoder)
    logger.info(f"Metadata saved for {crypto_symbol}.")
    model_filename = os.path.join(models_dir, f"{crypto_symbol}5mi_rmse3_high_model.joblib")
...
def process_crypto(crypto_symbol):
    study_name = "5min_rmse3_high" + crypto_symbol
    try:
    ...
    df = df[:-1]
    X = df.drop(['timestamp', 'high', 'future_high', 'predicted_high'], axis=1, errors='ignore')
    y = df['future_high']
