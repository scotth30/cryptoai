...
def ensure_predicted_close_column(engine, table_name):
    metadata = MetaData()
    try:
    ...
    if 'predicted_close' not in table.c:
        # Correctly wrap the ALTER TABLE statement in text() for execution
        alter_stmt = text(f"ALTER TABLE {table_name} ADD COLUMN predicted_close FLOAT;")
        with engine.begin() as conn:  # Use a transaction for DDL statements
            conn.execute(alter_stmt)
        logger.info(f"Column 'predicted_close' added to {table_name}.")
        return True
    return False

def save_predictions_to_db(predictions, crypto_symbol, timestamps):
    table_name = f"{crypto_symbol.lower()}10mi"  # Assuming predictions are saved back to the main table
    # Ensure the predicted_close column exists
    if not ensure_predicted_close_column(engine, table_name):
        return
    with engine.begin() as conn:  # Using a transaction for safety
        for timestamp, prediction in zip(timestamps, predictions):
            ...
            stmt = text(f"""
                UPDATE {table_name}
                SET predicted_close = :prediction
...
def save_metadata(best_model, X, y, crypto_symbol, chosen_objective):
    ...
    # Skipping MSLE computation due to negative target values
    
    metadata_path = os.path.join(output_dir, f"{crypto_symbol}10mi_{chosen_objective}_close_metadata.json")
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=4, cls=NumpyEncoder)
    logger.info(f"Metadata saved for {crypto_symbol}.")
    model_filename = os.path.join(models_dir, f"{crypto_symbol}10mi_{chosen_objective}_close_model.joblib")
...
def preprocess_data(df, crypto_symbol):
    # Separate the timestamp and 'close' columns
...
def process_crypto(crypto_symbol, chosen_objective):
    model_filename = os.path.join(models_dir, f"{crypto_symbol}10mi_{chosen_objective}_close_model.joblib")
    study_name = "10min_" + chosen_objective + "_close_" + crypto_symbol
    ...
    X = df.drop(['timestamp', 'close', 'future_close'], axis=1)
    y = df['future_close']

    if os.path.exists(model_filename):
        # Load the existing model and continue training
        best_model = joblib.load(model_filename)
        logger.info(f"Found existing model for {crypto_symbol}. Continuing training.")
    else:
        # Create a new study and train a new model
        logger.info(f"No existing model found for {crypto_symbol}. Starting training.")
    ...
    def objective(trial):
            # Define and return the trial's objective based on the parametrization
        param = {
        ...
        try:
            ...
            score = 1.0  # Assign a close score to indicate failure
        return score
    ...
    save_metadata(best_model, X, y, crypto_symbol, chosen_objective)
...
def main():
    for crypto_symbol in cryptocurrencies:
        ...
        for chosen_objective in objectives:
            logger.info(f"Using objective: {chosen_objective}")
            process_crypto(crypto_symbol, chosen_objective)
