...
def save_metadata(best_model, X, y, crypto_symbol):
    ...
    metadata_path = os.path.join(output_dir, f"{crypto_symbol}5mi_rmse3_metadata.json")
    with open(metadata_path, 'w') as f:
        json.dump(metadata, f, indent=4, cls=NumpyEncoder)
    logger.info(f"Metadata saved for {crypto_symbol}.")
    model_filename = os.path.join(models_dir, f"{crypto_symbol}5mi_rmse3_model.joblib")
...
def process_crypto(crypto_symbol):
    study_name = "5min_rmse3_" + crypto_symbol
    try:
    ...
    df = df[:-1]
    X = df.drop(['timestamp', 'close', 'future_close', 'predicted_close'], axis=1, errors='ignore')
    y = df['future_close']
